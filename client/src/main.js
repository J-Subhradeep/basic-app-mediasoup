...
const deviceSetup = async () => {
    ...
    console.log('[C1] router caps mimeTypes:',  (routerRtpCapabilities.codecs || []).map(c=>c.mimeType));
    await device.load({ routerRtpCapabilities })
    console.log(device.loaded)
    console.log('[C2] device loaded?', !!device.loaded);
console.log('[C2] device caps mimeTypes:',(device.rtpCapabilities.codecs || []).map(c=>c.mimeType));
...
const createConsumer = async()=>{
    //ask the socket.io server (signaling) for transport information
    console.log("hit create consumer");
